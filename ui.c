...
void show_stats(void) {
    ...
    printf("══════════════════════════════════\n");
    char buf[512];
    snprintf(buf, sizeof(buf), "echo '   🍅 Pomos completed : '; grep -c '\\[POMO\\]' \"%s\" || echo 0", log); system(buf);
    snprintf(buf, sizeof(buf), "echo '   ✅ Tasks completed : '; grep -c '\\[DONE\\]' \"%s\" || echo 0", log); system(buf);
    snprintf(buf, sizeof(buf), "echo '   📝 Notes logged    : '; grep -c '\\[NOTE\\]' \"%s\" || echo 0", log); system(buf);
    snprintf(buf, sizeof(buf), "echo '   🏆 Wins & wins     : '; grep -c '\\[WIN\\]'  \"%s\" || echo 0", log); system(buf);
    printf("══════════════════════════════════\n");
}
...
void show_search(const char* term) {
    ...
    snprintf(cmd, sizeof(cmd),
        "{ grep -i -H --color=always \"%s\" \"%s\" \"%s\" 2>/dev/null || true; } | sort -u || echo '   No matches found'",
        term, log, todo);
    system(cmd);
}